...
import com.pockettv.dropscreen.entity.ClingPositionResponse;
import com.pockettv.dropscreen.entity.ClingResponse;
import com.pockettv.dropscreen.entity.ClingVolumeResponse;
...
public class ClingPlayControl implements IPlayControl {
     ...
     * 每次接收 500ms 延迟
     */
    private static final int RECEIVE_DELAY = 200;
    /**
     * 上次设置音量时间戳, 防抖动
     */
    ...
    @Override
    public void playNew(final String url, final String name, final ControlCallback callback) {

        stop(new ControlCallback() { // 1、 停止当前播放视频
            @Override
            public void success(IResponse response) {

                setAVTransportURI(url, name, new ControlCallback() {   // 2、设置 url
                    @Override
                    public void success(IResponse response) {
     ...
     * @param callback 回调
     */
    private void setAVTransportURI(String url, String name, final ControlCallback callback) {
        if (Utils.isNull(url)) {
            return;
        }
